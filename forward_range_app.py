...
def render_simple(res):
    ...
    if res.get("spot_is_manual"):
        st.caption(f"Spot entered manually as {money(S0)} (the loaded data's last close was "
                   f"{money(res.get('data_close', S0))}). History, volatility and drift still "
                   f"come from the data — only the starting price is yours.")
...
def run_ui():
    ...
    _dc = float(st.session_state.get("last_data_close", 0.0))
    spot_mode = sb.radio("Spot price", ["From data (latest close)", "Enter manually"],
                         help="The historical series is reliable, but free feeds often lag the "
                              "latest close (especially BSE/NSE after hours). If the spot looks "
                              "stale, enter the current price yourself — only the projection's "
                              "starting point changes; all history/vol/drift stay from the data.")
    spot_override = None
    if spot_mode.startswith("Enter"):
        spot_override = sb.number_input("Current spot price", min_value=0.0,
                                        value=_dc, step=(50.0 if _dc >= 1000 else 1.0),
                                        help="Type the live price from your broker / exchange. "
                                             "Defaults to the last close loaded from the data.")
    ...
    r = np.diff(np.log(close))
    data_close = float(close[-1])
    st.session_state["last_data_close"] = data_close   # pre-fills the manual-spot box next run
    S0 = float(spot_override) if (spot_override and spot_override > 0) else data_close
    spot_is_manual = bool(spot_override and spot_override > 0 and
                          abs(spot_override - data_close) > 1e-9)
    ...
    if simple:
        render_simple(dict(
            ...
            spot_is_manual=spot_is_manual, data_close=data_close,
    ...
    # ---- header ----
    c = st.columns(4)
    c[0].metric("Spot (manual)" if spot_is_manual else "Spot (last close)", f"{S0:,.2f}",
                help=(f"set manually; the data's last close was {data_close:,.2f}"
                      if spot_is_manual else "last close from the loaded series"))
