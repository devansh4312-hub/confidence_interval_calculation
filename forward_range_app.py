...
def oos_calibrated_band(close, H, alpha, lookback, step, r_f, q, S0, sigH_now, mu_d_now,
                        regime=False, shrink=True, vol_kind="EWMA", burn=60, max_windows=500):
    ...
    cal_vol = vol_kind if vol_kind in ("EWMA", "EWMA+VoV", "Constant") else "EWMA+VoV"
    a = lookback
    while a <= len(rets)-H:
        tr = rets[a-lookback:a]; S = close[a]; Rreal = math.log(close[a+H]/S)
        cv, fc, _, _ = vol_model(tr, cal_vol); sigE = integrated_sigma(fc(H))
        mu_s = drift_daily(tr, "Shrunk", r_f, q)
        if sigE > 0:
    ...
    return dict(lo=S0*math.exp(mu_d_now*H + qlo*sigH_now),
                ...
                q_lo=qlo, q_hi=qhi, n=len(sel), regime=regime, shrunk=shrink)
...
def simple_fan_chart(S0, mu_d, sigH, H, mult_lo, mult_hi, unit=""):
    ...
    zones = [(mult_lo, mult_hi, "#1c3354", "rarely leaves"),
             (-1.282, 1.282, "#2c5fa0", "unlikely"),
             (-0.674, 0.674, "#4f9bef", "most likely")]
...
def run_ui():
    ...
    with sb.expander("⚙  Model settings", expanded=not simple):
        vol_kind = st.selectbox("Volatility model",
                                ["EWMA", "EWMA+VoV", "GARCH(1,1)", "GJR-GARCH(1,1)",
                                 "Blend (accuracy-wtd)", "Constant"], index=1,
                                help="EWMA+VoV (default) measured best out-of-sample with "
                                     "calibration; GJR adds the leverage effect; Blend "
                                     "accuracy-weights several models (measured no better, so "
                                     "not default); Constant = flat sd")
    ...
    with sb.expander("🎯  Evaluate an option", expanded=False):
        do_opt = st.checkbox("Enable option / strike evaluation")
        if do_opt:
            strike = st.number_input("Strike", value=25000.0, step=50.0)
            ...
            premium = st.number_input("Premium (0 = skip P&L)", 0.0, step=0.5)
    ...
    try:
        oos_hl = oos_calibrated_band(close, H, alpha, 252, 5, r_f, dy, S0, sigH, mu_d,
                                     regime=False, vol_kind=vol_kind)
    except Exception:
        oos_hl = None
        ...
        if len(close) < lookback + H + 30:
            ...
            if calib:
                ...
                oos = oos_calibrated_band(close, H, alpha, int(lookback), int(bstep),
                                          r_f, dy, S0, sigH, mu_d, regime=False, vol_kind=vol_kind)
                if oos:
