...
def vol_model(r, kind, quick=False, daily_var=None):
    ...
    if kind == "EWMA-Range (Garman-Klass)":
        ...
        cv = ewma_vol(r); cur = float(cv[-1])       # no OHLC -> graceful fallback
        return cv, (lambda H, c=cur: np.full(H, c)), cur, "EWMA (λ=0.94) — no OHLC, fell back to close"
    if kind == "GARCH(1,1)":
...
def strategy_pnl_fan(paths, legs, lot, expiry_days, r, q, n_marks=12):
    """Day-by-day P&L distribution of the position from today to expiry (B1).
    Reprices every leg via BS along each simulated path at frozen entry IV.
    Returns (day_axis_calendar, pct dict of P&L percentile curves)."""
    ...
    for di in idx:
        T_rem = max((expiry_days - days_cal[list(idx).index(di)])/365.0, 0.0)
        S = paths[:, di]
        val = np.zeros(n)
        for lg in legs:
            s = 1 if lg["side"] == "buy" else -1
            val += s*lg["qty"]*lot*bs_price_vec(S, lg["K"], T_rem, r, q, lg.get("iv", 0.2), lg["kind"])
...
def run_ui():
    ...
    with sb.expander("⚙  Model settings", expanded=not simple):
        vol_kind = st.selectbox("Volatility model",
                                ["EWMA", "EWMA+VoV", "EWMA-Range (Garman-Klass)", "GARCH(1,1)",
                                 "GJR-GARCH(1,1)", "Blend (accuracy-wtd)", "Constant"], index=1,
                                help="EWMA+VoV (default) measured best out-of-sample with "
                                     "calibration; EWMA-Range uses OHLC (Garman-Klass) and measured "
                                     "slightly sharper bands when Open/High/Low/Close is available; "
                                     "GJR adds leverage; Blend accuracy-weights several models "
                                     "(measured no better, so not default); Constant = flat sd")
    ...
    if vol_kind.startswith("EWMA-Range") and daily_var is None:
        st.info("No OHLC available for this data — range (Garman-Klass) vol needs Open/High/"
                "Low/Close, so it fell back to close-to-close EWMA.")
    ...
    if do_opt and strat is not None:
      with st.spinner("Evaluating strategy (payoff, Greeks, P&L fan)…"):
        ...
        def mk(kind, side, K, qty=1, prem_override=0.0):
            iv = iv_for(K)
            prem = (float(prem_override) if prem_override and prem_override > 0
                    else bs_price_and_prob(S0, K, T, r_f, dy, iv, kind)["price"])
        ...
        d_ax, fan = strategy_pnl_fan(paths, legs, int(lot), days, r_f, dy)
        st.markdown("**P&L over time** (today → expiry, IV held at entry)")
        ...
        st.caption("How the position's mark-to-market P&L is likely to evolve day by day — the "
                   "band is the 5–95% / 25–75% spread of simulated paths. Assumes implied vol "
                   "stays at entry; a real vol spike widens it (especially for short-vol legs).")
